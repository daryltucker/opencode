  ...
  Effect.gen(function* () {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const interpolationErrors: ConfigMarkdown.InterpolationError[] = []
        ...
        for (const dir of directories) {
          ...
          const agentResult = yield* Effect.promise(() => ConfigAgent.load(dir))
          result.agent = mergeDeep(result.agent ?? {}, agentResult.agents)
          interpolationErrors.push(...agentResult.errors)
          const modeResult = yield* Effect.promise(() => ConfigAgent.loadMode(dir))
          result.agent = mergeDeep(result.agent ?? {}, modeResult.agents)
          interpolationErrors.push(...modeResult.errors)
        ...
        if (interpolationErrors.length > 0) {
          for (const err of interpolationErrors) {
            log.warn("file interpolation failed", { type: err.type, refPath: err.refPath, resolvedPath: err.resolvedPath })
          }
          const summary = `${interpolationErrors.length} file reference${interpolationErrors.length > 1 ? "s" : ""} could not be resolved`
          const details = interpolationErrors
            .slice(0, 5)
            .map((e) => {
              const reason = e.type === "circular" ? "circular" : e.type === "max_depth" ? "max depth" : "not found"
              return `• ${e.refPath} (${reason})`
            })
            .join("\n")
          const message = interpolationErrors.length > 5
            ? `${details}\n• ...and ${interpolationErrors.length - 5} more`
            : details
          yield* bus.publish(TuiEvent.ToastShow, {
            title: "Agent file interpolation",
            message: `${summary}\n${message}`,
            variant: "warning",
            duration: 10000,
